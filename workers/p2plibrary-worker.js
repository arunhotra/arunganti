/**
 * P2P Library Worker - Cloudflare Worker with Google Drive integration
 * Handles book uploads and library gallery for peer-to-peer book sharing
 *
 * Environment Variables (set via wrangler secret put):
 * - GOOGLE_CLIENT_ID: OAuth client ID
 * - GOOGLE_CLIENT_SECRET: OAuth client secret
 * - GOOGLE_REFRESH_TOKEN: OAuth refresh token
 * - GOOGLE_DRIVE_FOLDER_ID: Google Drive folder ID for "P2P Library Books"
 * - DELETION_SECRET: Password for deleting books
 *
...
export default {
    async fetch(request, env, ctx) {
        ...
        try {
            ...
            } else if (path.startsWith('/toggle-borrowed/') && request.method === 'PATCH') {
                response = await handleToggleBorrowed(request, path, env);
            } else {
...
async function handleUpload(request, env) {
    ...
    // Validation
    if (!imageFile || !title || !author || !summary) {
        return new Response(
            JSON.stringify({ error: 'Missing required fields: image, title, author, or summary' }),
    ...
    if (title.length > CONFIG.maxTitleLength) {
        return new Response(
            JSON.stringify({ error: `Title too long. Maximum ${CONFIG.maxTitleLength} characters.` }),
            { status: 400, headers: { 'Content-Type': 'application/json' } }
        );
    }

    if (author.length > CONFIG.maxAuthorLength) {
        return new Response(
            JSON.stringify({ error: `Author name too long. Maximum ${CONFIG.maxAuthorLength} characters.` }),
            { status: 400, headers: { 'Content-Type': 'application/json' } }
        );
    }
    ...
    if (summary.length > CONFIG.maxSummaryLength) {
        return new Response(
            JSON.stringify({ error: `Summary too long. Maximum ${CONFIG.maxSummaryLength} characters.` }),
            { status: 400, headers: { 'Content-Type': 'application/json' } }
        );
    }

    if (notes.length > CONFIG.maxNotesLength) {
        return new Response(
            JSON.stringify({ error: `Notes too long. Maximum ${CONFIG.maxNotesLength} characters.` }),
            { status: 400, headers: { 'Content-Type': 'application/json' } }
        );
    }
...
async function handleDelete(request, path, env) {
    try {
        ...
        return new Response(
            JSON.stringify({ error: error.message || 'Failed to delete book' }),
            {
                status: 500,
                headers: { 'Content-Type': 'application/json' }
            }
...
// ============================================================================
// Toggle Borrowed Status Handler
// ============================================================================

async function handleToggleBorrowed(request, path, env) {
    try {
        // Extract item ID from path
        const itemId = path.split('/toggle-borrowed/')[1];
        ...
        if (typeof borrowed !== 'boolean') {
            return new Response(
                JSON.stringify({ error: 'Invalid borrowed status. Must be true or false.' }),
                {
                    status: 400,
                    headers: { 'Content-Type': 'application/json' }
                }
        ...
        return new Response(
            JSON.stringify({ error: error.message || 'Failed to update borrowed status' }),
            {
                status: 500,
                headers: { 'Content-Type': 'application/json' }
            }
